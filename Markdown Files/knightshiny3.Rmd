...
# NOTE: This file was updated to test out the anti-saccade task
# It also hasn't been updated to reflect the new measurement function yet
#It should be updated to let a person upload a block and see the results: TO DO 3-28-2018
...
# hh<- readRDS('knightdemo4.RDS')
hh<-readRDS('AntiSaccadeTest3.RDS')
...
plotTrial<- function(hh){
  ...
  gp<- ggplot(hh)+
    ...
    geom_line(aes(tcounter,Targ),size=2,alpha=0.5)+
...
renderPlotly({
    ...
    p<-plotTrial(filter(hh,trialnum==trialnums[chosentrial],block==blockchoice))
    print(ggplotly(p))
    # 
    # ggplot(filter(hh,trialnum==chosentrial))+
